...
public class AdminShipmentquery {
	public void adminShipmentquery() {
		Scanner adminInput = new Scanner(System.in);
		Order order = new Order();
		System.out.println("欢迎进入出货查询页面！");
		...
		try{
			...
			ResultSet res = ps.executeQuery("SELECT * FROM SHIPMENT");
			if(res.next()){
				System.out.println("查询成功，信息如下");
				System.out.println("货物编号："+res.getInt("id")+"\t手机名称："+res.getString("phonename")+"\t个数信息："+res.getString("number")+"\t价格信息："+res.getString("price")+"\t更改时间："+res.getString("ts"));
			}else{
